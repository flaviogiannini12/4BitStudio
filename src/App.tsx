...
const pages: AppPage[] = ['home','clients','projects','tasks','payments','team']
type CreateKind = 'client' | 'project' | 'task' | 'payment' | 'recurrence' | 'member'
...
const titles: Record<AppPage, { title: string; eyebrow: string }> = {
  ...
  projects: { title: 'Dal brief al go-live.', eyebrow: 'Progetti' },
  ...
  return <>
    ...
    <main className="app-shell">
      ...
      {studio.loading ? <LoadingSkeleton/> : <>
        ...
        {page === 'projects' && <ProjectsPage data={studio.data} actions={studio.actions} onNew={() => setCreateKind('project')}/>} 
